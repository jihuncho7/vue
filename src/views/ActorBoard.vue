    ...
    <main>
      ...
      <div class="communitybtn-group" role="group">
        <router-link to="/HireStaff">
          <button class="btnhsa" type="button">스탭 구인</button>
        </router-link>
        ...
        <router-link to="/ActorBoard">
          <button class="btnha" type="button">액터스 구직</button>
        </router-link>
      </div>
      ...
      <div class="header">
        ...
        <!-- 좌우 스크롤 버튼 -->
        <nav>
          <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none" @click="$refs.StreamList.prev()">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none" @click="$refs.StreamList.next()">
      ...
      <!-- 신규배우프로필 목록 -->
      <vue-horizontal ref="StreamList" class="horizontal" :button="false">
        <div class="item" v-for="item in axiosget_staff" :key="item.id">
          <div class="overlay">
            <div class="image" @click="goToView(item)" :style="{background: `url(${item.image})`}">
            </div>
          </div>
      ...
      <section id="contentsBox">
        <h2 style="color:white;">배우 프로필 모아보기</h2>

        <div class="profileBox" style="color:white;">
          <ul class="profiles" v-for="profile in axiosget_staff" :key="profile.id">
            <li><img :src="profile.image" style="width:100%; height:300px;"></li>
            <li>{{ profile.name }}</li>
            <li>{{ profile.age }}세</li>
...
export default {
  ...
  // main.js로부터 데이터 호출하는 영역
  created() {
    axios({
      method: "GET",
      url: "http://127.0.0.1:8000/film/resumeactorboard"
    })
        .then(response => {
          this.axiosget_staff = response.data;
          console.log( this.axiosget_staff);
        })
        .catch(response => {
  ...
  methods:{
    goToView: function(viewData) {
      this.$router.push({name: 'ActorBoardDetail',
        params: {
          id: viewData.id,
          hit: viewData.hit,
        }});
      axios({
        method: "PATCH",
        url: "http://127.0.0.1:8000/film/resumeactorboard/" + viewData.id + "/",
        data: {
          hit: viewData.hit + 1,
        },
      })
...
<!-- Container 역할의 main을 화면 크기마다 padding 설정 -->
<style scoped>
main {
  padding: 24px;
}
