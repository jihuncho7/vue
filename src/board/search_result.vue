...
<div >
      <b-container fluid="lg" class="freeboardHeader">
        <b-row class="justify-content-md-center">
          <b-col cols="5">
            <h2 style="font-size:24px">전체 검색 결과</h2>
          </b-col>
          <b-col cols="4" class="searchbar">
    ...
    <div class="freeboardList">
      <div class="boardtop d-flex mw-100 mh-100 float-right" style=" width: 800px;">
        <b-input-group class="">
          <template #prepend>
        ...
        <router-link to="/FreeWrite" class="w-100">
          <b-button variant="primary" style="border-radius: 14px" v-bind:propsdata="freeboard">글쓰기</b-button>
        </router-link>
      </div>
    <table align="center" id="tableList">
        ...
        <tr class="table_records" align="center" v-for="(item, index) in freeboard.freeboard" :key="index">
...
export default {
  ...
  created() { // 속성에 함수가 붙으면(예를 들면 created: function(){} 요놈) 줄여서 쓴다. ES6 축약문법.
    ...
    axios({
      ...
      method: "POST",
      url: "http://127.0.0.1:8000/film/SearchAllView",
      data:"\""+ this.$route.query.search_q+ "\"",
    })
        .then(response => {
